...
// Network Identifiers
const char* const HOSTNAME         = "esp32-classroom"; // Accessible at http://esp32-classroom.local
const char* const FIRMWARE_VERSION = "2.3.0";
...
const char* const SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Iml5bnVmemhvcGNyZGFkdGx1cW54Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODkzNjMwOTEsImV4cCI6MjEwNDkzOTA5MX0.JqwsP1SB7gAHCFu_Ed60PU0MOmdPakFmD0KNtZMXqa4";

// Cloud Polling & Telemetry Frequencies
const unsigned long SUPABASE_POLL_INTERVAL_MS      = 1000; // Poll device state changes from cloud every 1.0s
const unsigned long SUPABASE_TELEMETRY_INTERVAL_MS = 4000; // Push one rotating telemetry slot every 4.0s
...
// (Avoid GPIO 12 if your relay module has active pull-ups, as it's a boot-strapping pin)
#define RELAY_CORRIDOR_LIGHT1 16 // Corridor 1 lights relay (Set to 12 if strictly using old wiring)
#define RELAY_CORRIDOR_LIGHT2 17 // Corridor 2 lights relay (Set to 13 if strictly using old wiring)
...
const int SERVO_OPEN_ANGLE            = 180;    // Curtains Open (90 deg)
const int SERVO_SPEED_MS              = 25;    // Milliseconds per degree sweep
const unsigned long SERVO_IDLE_DETACH = 800;   // Detach PWM after movement to prevent buzzing
